...
int buzzer = 9;      // the buzzer to wake you up
...
//This routine occurs when you push twice and hold the button down
...
void setAlarm(void)
{
  ...
  while(idleMiliseconds < 2000) {
    ...
    //Update the minutes and hours variables
    alarm_hours += alarm_minutes / 60; //12 + (96 / 60) = 13
    alarm_minutes %= 60; //minutes = 36

    //Do we display 12 hour or 24 hour time?
    if(TwelveHourMode == TRUE) {
      //In 12 hour mode, hours go from 12 to 1 to 12.
      while(alarm_hours > 12) alarm_hours -= 12;
    }
    else {
      //In 24 hour mode, hours go from 0 to 23 to 0.
      while(alarm_hours > 23) alarm_hours -= 24;
    }
    ...
    if(digitalRead(theButton) == LOW) {
      ...
      buttonHold++;
      if(buttonHold < 10)
        alarm_minutes++; //Advance the minutes
      else {
        ...
        alarm_minutes /= 10; //minutes = 46 / 10 = 4
        alarm_minutes *= 10; //minutes = 4 * 10 = 40
        alarm_minutes += 10;  //minutes = 40 + 10 = 50
      }
    }
    else
      buttonHold = 0;
...
#define DIGIT_OFF  HIGH
  ...
  for(int digit = 4 ; digit > 0 ; digit--) {
    ...
    case 2:
      ...
      if(displayColon == TRUE) digitalWrite(colons, HIGH); //When we update digit 2, let's turn on colons as well
      break;
    case 3:
...
//Take a temperature between -99 and 99 degrees and display it
// on four characters. Add 'degree' symbol and optional minus
// sign in front.
